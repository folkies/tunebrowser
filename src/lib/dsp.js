...
function RFFT(bufferSize, sampleRate) {
  ...
  this.reverseBinPermute = function (dest, source, startAt) {
    ...
    dest[0] = source[startAt];

    do {
      r += halfSize;
      dest[i] = source[startAt + r];
      dest[r] = source[startAt + i];
      ...
      if (r >= i) {
        ...
        dest[nm1-i] = source[startAt + nm1 - r];
        dest[nm1-r] = source[startAt + nm1 - i];
      }
      i++;
    } while (i < halfSize);
    dest[nm1] = source[startAt + nm1];
  };
...
RFFT.prototype.forward = function(buffer, startAt) {
  startAt = typeof startAt !== 'undefined' ? startAt : 0;
  ...
  this.reverseBinPermute(x, buffer, startAt);
...
WindowFunction.prototype.process = function(buffer, startAt, length) {
  startAt = typeof startAt !== 'undefined' ? startAt : 0;
  length = typeof length !== 'undefined' ? length : buffer.length;
